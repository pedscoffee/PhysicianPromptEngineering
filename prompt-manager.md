...
<script>
        ...
        filteredSnippets.forEach(snippet => {
            ...
            card.innerHTML = `
                ...
                <div class="snippet-actions">
                    ...
                    <button class="btn btn-sm btn-secondary" onclick="copySnippet(${snippet.id}, event)">
    ...
    window.copyFromView = function(event) {
        navigator.clipboard.writeText(currentViewContent).then(() => {
            const btn = event.currentTarget || (event.target && event.target.closest ? event.target.closest('button') : null);
            if (btn) {
    ...
    // COPY SNIPPET
    // =====================================================
    window.copySnippet = function(id, event) {
        const snippet = snippets.find(s => s.id === id);
        if (!snippet) return;
