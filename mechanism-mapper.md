...
<script>
    ...
    window.switchMode = function(mode) {
        ...
        if (mode === 'ai') {
            ...
            // Populate manual editor with existing output if available
            const existingSvg = document.querySelector('#mermaid-output svg');
            if (existingSvg && !document.getElementById('mermaidCode').value) {
                // Extract mermaid code from existing diagram if possible
                // For now, just show a helpful message
                const statusBar = document.getElementById('statusBar');
                statusBar.style.display = 'block';
                statusBar.textContent = 'Tip: You can edit the generated code or start fresh!';
